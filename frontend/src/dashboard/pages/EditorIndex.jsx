...
const EditorIndex = () => {
    const [stats, setStats] = useState({
        totalNews: 0,
        submittedNews: 0,
        reviewedByWriterNews: 0,
        reviewedByEditorNews: 0,
        publishedNews: 0,
        deactiveNews: 0
    })
    ...
    const get_stats = async () => {
        try {
            const { data } = await axios.get(`${base_url}/api/editor/stats`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('newsToken')}`
                }
            })
    ...
    return (
        <div className='mt-2'>
            <div className='grid grid-cols-6 gap-x-4 mb-6'>
                ...
                <div className='w-full p-8 flex justify-center flex-col rounded-md items-center gap-y-2 bg-white text-slate-700'>
                    <span className='text-xl font-bold'>{stats.submittedNews}</span>
                    <span className='text-md'>Submitted</span>
                </div>
                <div className='w-full p-8 flex justify-center flex-col rounded-md items-center gap-y-2 bg-white text-slate-700'>
                    <span className='text-xl font-bold'>{stats.reviewedByWriterNews}</span>
                    <span className='text-md'>Reviewed by Writer</span>
                </div>
                <div className='w-full p-8 flex justify-center flex-col rounded-md items-center gap-y-2 bg-white text-slate-700'>
                    <span className='text-xl font-bold'>{stats.reviewedByEditorNews}</span>
                    <span className='text-md'>Reviewed by Editor</span>
                </div>
                <div className='w-full p-8 flex justify-center flex-col rounded-md items-center gap-y-2 bg-white text-slate-700'>
                ...
                <div className='w-full p-8 flex justify-center flex-col rounded-md items-center gap-y-2 bg-white text-slate-700'>
                    <span className='text-xl font-bold'>{stats.deactiveNews}</span>
                    <span className='text-md'>Deactive</span>
                </div>
            </div>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {news.map((n, i) => (
                        <div key={i} className='border border-gray-200 rounded-md p-4 cursor-pointer' onClick={() => view_news(n._id)}>
                            <img className='w-full h-64 object-top rounded-md mb-2' src={n.image} alt={n.title} />
                            ...
                            <div className='flex justify-start items-center gap-x-2 mt-2'>
                                {n.status === 'reviewed_by_writer' && (
                                    <span onClick={(e) => { e.stopPropagation(); update_status('reviewed_by_editor', n._id, 'Reviewed by Editor') }} className='px-2 py-1 bg-blue-100 text-blue-800 rounded-lg text-xs cursor-pointer hover:bg-blue-200'>
                                        {res.loader && res.id === n._id ? 'Loading...' : 'Review'}
                                    </span>
                                )}
                                {n.status === 'rework_needed' && n.returnTo === 'editor' && (
                                    <span onClick={(e) => { e.stopPropagation(); update_status('reviewed_by_editor', n._id, 'Reworked by Editor') }} className='px-2 py-1 bg-yellow-100 text-yellow-800 rounded-lg text-xs cursor-pointer hover:bg-yellow-200'>
                                        {res.loader && res.id === n._id ? 'Loading...' : 'Rework'}
                                    </span>
                                )}
                                {(n.status === 'reviewed_by_writer' || n.status === 'rework_needed') && (
                                    <span onClick={(e) => { e.stopPropagation(); update_status('rejected', n._id, 'Rejected by Editor') }} className='px-2 py-1 bg-red-100 text-red-800 rounded-lg text-xs cursor-pointer hover:bg-red-200'>
                                        {res.loader && res.id === n._id ? 'Loading...' : 'Reject'}
                                    </span>
                                )}
                            </div>
                        </div>
